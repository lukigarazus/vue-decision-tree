<template>
  <div class="decision-tree">
    ...
    <div class="horizontal-margin">
      <Button style="width: 100%" :loading="saving" @click="saveTree" type="primary">Save this tree</Button>
    </div>
  </div>
</template>
...
export default {
  props: ["db"],
  data() {
    return { ...initializeTree(), optionModal: false, saving: false };
  },
  methods: {
    async saveTree() {
      this.saving = true;
      await this.db.trees.put(this.tree);
      this.$emit('create')
      setTimeout(() => {
        const newTree = initializeTree();
        Object.keys(newTree).forEach(key => this[key] = newTree[key]);
        this.saving = false;
      }, 2000);
    },
    createNew() {
